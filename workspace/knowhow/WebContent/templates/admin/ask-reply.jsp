...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/static/css/admin/ask-reply.css">
<link
				...
				<div class="user-list-contents">
					<div class="member-input-wrap">
						<div class="member-delete-button-wrap">
							<input class="comment-delete-button" type="submit" value="댓글 삭제">
						</div>

						<div class="member-search-wrap">
							<div class="search-border">
								<form
									action="${pageContext.request.contextPath}/adminQuestionCommentListAction.admin"
									name="search-form">
									<img id="search-icon" alt="search"
										src="${pageContext.request.contextPath}/images/search-icon.png">

									<input class="member-search" type="text" name="commentContentKeyword"
										value="${commentContentKeyword}" "placeholder="질문 내용 검색">
								</form>
							</div>
						</div>
					</div>
					...
					<div class="total-mentor">
						<p>전체 댓글</p>
						<p class="total-mentor-num">${questionCommentTotal}</p>
					</div>
					...
					<div class="checkbox-group">
						<div class="mentor-list-header">
							...
							<div class="ask-reply-num">댓글 번호</div>
							<div class="ask-num">질문 번호</div>
							<div class="specialization">전문 분야</div>
							<div class="user-role">구분</div>
							<div class="user-nick-name">작성자</div>
							<div class="ask-reply-contents">댓글 내용</div>
							<div class="user-ask-reply-day">작성일</div>
							<div class="detail"></div>
						</div>

						<!-- 여기부터 내용 -->
						<c:forEach var="comment" items="${questionComments}">
							<div class="mentor-list-contents">
								<div class="checkbox-zip">
									<input type="checkbox" id="check1" class="normal" value="">
								</div>
								<div class="ask-reply-num">${comment.commentId}</div>
								<div class="ask-num">${comment.questionId}</div>
								<div class="specialization">${comment.categoryName}</div>
								<div class="user-role"><c:out
											value="${comment.resumeExistence == false ? '멘티' : '멘토'}" /></div>
								<div class="user-nick-name">${comment.memberNickname}</div>
								<div class="ask-reply-contents">${comment.commentContent}
								</div>
								<div class="user-ask-reply-day">${comment.commentRegdate}
								</div>
								<div class="detail">
									<button class="detail-button modal1" type="submit" onclick="openModal(${comment.commentId})">
										<img class="detail-icon" src="./images/detail-icon.png">
									</button>
								</div>
								<div id="my_modal${comment.commentId}" class="my_modal">
									<a class="modal_close_btn">✖</a>
									<div class="comment-title">작성자: ${comment.memberNickname}</div>
						...
						<section id="content-wrap">
							...
							<div id="paging-wrap">
								<c:if test="${prev}">
									<a
										href="javascript:location.href='adminQuestionCommentListAction.admin?page=${startPage - 1}'"
								...
								<c:forEach var="i" begin="${startPage}" end="${endPage}">
									<c:choose>
										...
										<c:otherwise>
											<a
												href="javascript:location.href='adminQuestionCommentListAction.admin?page=${i}'"
								...
								<c:if test="${next}">
									<a
										href="javascript:location.href='adminQuestionCommentListAction.admin?page=${endPage + 1}'"
										class="paging paging-move"><img
...
<script>
	let questionComments = `${questionComments}`; 
